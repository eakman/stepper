	...
	document.addEventListener("DOMContentLoaded", function(event) {
	  ...
	  const modulate = (steps) => {
	    ...
	    Array.from($('.step')).forEach((step) => {
	      ...
	      // console.log(`start ${step.attributes.note.value}`);
	      step.attributes.note.value = allNotes[(startNote + steps) % 72];
	      // console.log(`end ${step.attributes.note.value}`);
	    });
	  ...
	  $( function() {
	    $( ".pitch-fader" ).slider({
	      ...
	      slide: function(event, ui) {
	        console.log(ui.value - 12);
	        modulate(ui.value - 12);
	      }
	    });
	  });
